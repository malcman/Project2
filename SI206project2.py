...
def find_urls(s):
    return re.findall('(https?:[^\s]+\.[^\s][^\s]+)',s)
...
def grab_headlines():
    result = requests.get('http://www.michigandaily.com/section/opinion')
    soup = BeautifulSoup(result.content, "lxml")
    headlinesData = soup.find_all("ol")
    headlines = []
    for story in headlinesData[0].find_all('a'):
